...
Note that adding assembly references requires **.NET Framework 4.8 on the build machine**, because the SQL CLR APIs in DacFx are only available on .NET Framework. Builds on Linux/macOS that include assembly references are therefore not supported.
