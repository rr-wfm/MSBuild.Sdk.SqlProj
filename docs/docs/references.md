...
Any `.dll` flowing into the build via `<Reference>`, `<ProjectReference>`, or as a sibling of a referenced `.dacpac` is picked up automatically and added to the dacpac as a `CREATE ASSEMBLY` object. The default permission set is `SAFE`; override it with the `AssemblyPermissionSet` MSBuild property (`Safe`, `ExternalAccess`, or `Unsafe`):

```xml
<PropertyGroup>
  <AssemblyPermissionSet>Unsafe</AssemblyPermissionSet>
</PropertyGroup>
```
...
- **.NET Framework 4.8 must be installed on the build machine.** The SDK shells out to a small Windows-only helper (`DacpacToolFramework.exe`, targeting `net48`) to add the assemblies to the dacpac, because the SQL CLR APIs in DacFx are only available on .NET Framework. Builds on Linux/macOS that include assembly references are therefore not supported.
